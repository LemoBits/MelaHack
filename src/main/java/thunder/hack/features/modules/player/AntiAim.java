...
public class AntiAim extends Module {
    ...
    public void onSync(EventSync e) {
        if(allowInteract.getValue() && (mc.options.keyAttack.isDown() || mc.options.keyAttack.isDown())) return;
        double gcdFix = (Math.pow(mc.options.sensitivity().get() * 0.6 + 0.2, 3.0)) * 1.2;
        if (yawMode.getValue() != Mode.None) {
            mc.player.setYRot((float) (rotationYaw - (rotationYaw - mc.player.getYRot()) % gcdFix));
            if (bodySync.getValue())
                mc.player.setYBodyRot(rotationYaw);
        }
        if (pitchMode.getValue() != Mode.None)
            mc.player.setXRot((float) (rotationPitch - (rotationPitch - mc.player.getXRot()) % gcdFix));
    }

    @EventHandler(priority = EventPriority.HIGH)
    public void onCalc(PlayerUpdateEvent e) {
        if (pitchMode.getValue() == Mode.RandomAngle)
            if (mc.player.tickCount % Speed.getValue() == 0)
        ...
        if (pitchMode.getValue() == Mode.Sinus) {
            ...
            rotationPitch = (float) (mc.player.getXRot() + pitchDelta.getValue() * Math.sin(pitch_sinus_step));
            rotationPitch = MathUtility.clamp(rotationPitch, -90, 90);
        }

        if (yawMode.getValue() == Mode.Sinus) {
            yaw_sinus_step += Speed.getValue() / 10f;
            rotationYaw = (float) ((mc.player.getYRot() + yawDelta.getValue() * Math.sin(yaw_sinus_step)) + yawOffset.getValue());
        }
        ...
        if (pitchMode.getValue() == Mode.Static) {
            rotationPitch = mc.player.getXRot() + pitchDelta.getValue();
            rotationPitch = MathUtility.clamp(rotationPitch, -90, 90);
        }
        if (yawMode.getValue() == Mode.Static)
            rotationYaw =  mc.player.getYRot() % 360 + yawDelta.getValue();


        // господи какое говно я написал
        if (pitchMode.getValue() == Mode.Jitter) {

            if (mc.player.tickCount % (Speed.getValue() * 2) == 0) {
                rotationPitch = pitchDelta.getValue() / 2f;
            }

            if (mc.player.tickCount % (Speed.getValue() * 2) == Speed.getValue()) {
                rotationPitch = pitchDelta.getValue() / -2f;
            }
        }
        ...
        if (yawMode.getValue() == Mode.Jitter) {
            ...
            if (mc.player.tickCount % (Speed.getValue() * 2) == 0) {
                rotationYaw = yawDelta.getValue() / 2f + (float) yawOffset.getValue() + mc.player.getYRot();
            }
