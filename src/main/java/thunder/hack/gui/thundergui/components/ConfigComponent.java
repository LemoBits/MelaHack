...
public class ConfigComponent {
    ...
    public void render(GuiGraphicsExtractor context, int MouseX, int MouseY) {
        ...
        if (Managers.CONFIG.currentConfig.getName().equals(name + ".th")) {
            Render2DEngine.drawGradientRound(context.pose(), posX + 5, posY, 285, 30, 4f,
        ...
        Render2DEngine.drawRound(context.pose(), posX + 250, posY + 8, 30, 14, 2f, Render2DEngine.applyOpacity(new Color(25, 20, 30, 255), getFadeFactor()));

        if (Render2DEngine.isHovered(MouseX, MouseY, posX + 252, posY + 10, 10, 10)) {
            Render2DEngine.drawRound(context.pose(), posX + 252, posY + 10, 10, 10, 2f, Render2DEngine.applyOpacity(new Color(21, 58, 0, 255), getFadeFactor()));
        } else
            Render2DEngine.drawRound(context.pose(), posX + 252, posY + 10, 10, 10, 2f, Render2DEngine.applyOpacity(new Color(32, 89, 0, 255), getFadeFactor()));
        ...
        FontRenderers.icons.drawString(context.pose(), "x", posX + 252, posY + 13, Render2DEngine.applyOpacity(-1, getFadeFactor()));
        FontRenderers.icons.drawString(context.pose(), "w", posX + 268, posY + 13, Render2DEngine.applyOpacity(-1, getFadeFactor()));


        FontRenderers.mid_icons.drawString(context.pose(), "u", posX + 7, posY + 5, Render2DEngine.applyOpacity(-1, getFadeFactor()));
        FontRenderers.modules.drawString(context.pose(), name, posX + 37, posY + 6, Render2DEngine.applyOpacity(-1, getFadeFactor()));
        FontRenderers.settings.drawString(context.pose(), "updated on: " + date, posX + 37, posY + 17, Render2DEngine.applyOpacity(new Color(0xFFBDBDBD, true).getRGB(), getFadeFactor()));
    }
